...
#### OBSERVATION:
```{r}
Identified the factors influencing proposal acceptance. Data cleaning and **exploratory analysis** revealed that while income showed no statistically significant impact on proposal response (P-value > 0.05), relational features like **RomanticGestureScore**, **CompatibilityScore**, and **CommunicationScore** played a crucial role. Among the models implemented—**Logistic Regression, Decision Tree, and Random Forest**—the **Random Forest model** performed best, achieving high accuracy, precision, recall, and AUC. Feature importance analysis further confirmed that interpersonal traits are stronger predictors of proposal acceptance than demographic variables like income. The results demonstrate the value of **machine learning** in uncovering patterns in human relationships and predicting real-world outcomes.
```

#### RESULT:
```{r}
The results of the analysis highlight that **income** is not a statistically significant factor in determining proposal acceptance, as confirmed by the **Chi-Square Test** (P-value > 0.05). Among the machine learning models implemented, the **Random Forest model** outperformed Logistic Regression and Decision Tree models, achieving the highest **accuracy, precision, recall, and AUC** scores. Feature importance analysis revealed that **RomanticGestureScore**, **CompatibilityScore**, and **CommunicationScore** are the most influential predictors of proposal acceptance. Incorporating additional features such as **Age**, **Income**, **Height**, and **DistanceKM** further enhanced the model's performance. The final **Random Forest model** demonstrates strong predictive capability, successfully capturing the key factors influencing proposal outcomes.
```
