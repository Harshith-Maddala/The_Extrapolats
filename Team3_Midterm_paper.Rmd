...
Next, we examined the dataset to identify any instances of empty strings or NA values. We used sapply() to apply a function to all columns in cupid_df to count empty strings and NA values. This has allowed us to gain a clear comprehension of the data quality by identifying the amount of empty strings and missing values in each column. 
...
After we found these problems, we substituted all absent strings with NA values. This simplified the management of data gaps. We confirmed again to ensure that all blank strings were changed to NA, making our dataset more reliable and consistent. 
...
We used subset() and complete.cases() to create a new dataframe, cleaned_cupid_df, by removing any incomplete cases. Removing rows with missing values was an essential step to guarantee the accuracy of our analysis. After cleaning, we double-checked for missing values to confirm that all rows were fully filled. 
...
Following that, we provided further clarification on the gender variable. Through the use of mutate() and recode(), we converted the abbreviated terms ('f' for female and 'm' for male) to complete words, which enhances the readability and comprehension of the data for future users.
...
Next, we converted multiple character columns into factors using the `as.factor()` function. This step is essential when working with categorical data in R because it helps with organization and allows for more accurate statistical analysis. Altering these columns was crucial for any forthcoming modeling we intended to conduct. 
...
After we examined the data, we condensed it with the `summary()` function. This enabled us to develop a thorough comprehension of the dataset, aiding us in understanding its distribution and characteristics before diving into a more in-depth analysis.
...
We created a histogram to show how age is distributed. We employed ggplot() to generate a chart with age on the x-axis, with a blue color palatte. This helped us in visualizing how ages were spread out in the dataset. 
The bar graph was made with a bin width of 5, and axes titles and labels were added for improved clarity. 
