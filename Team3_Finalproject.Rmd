...
### 4.3: Which classification model (logistic regression, decision tree, or random forest) is the most suitable for predicting whether a proposal will be accepted or rejected based on features RomanticGestureScore, CompatibilityScore, and DistanceKM?
...
# Random Forest Model
random_forest_model <- randomForest(Response ~ Height + Income + Age, data = trainData, ntree = 500, mtry = 2, importance = TRUE)
print(random_forest_model)
predicted_probabilities_rf <- predict(random_forest_model, proposal_df, type = "prob")[, 2]  # Probabilities for class 1
predicted_classes_rf <- predict(random_forest_model, proposal_df, type = "response")# Predicted classes
actual <- as.factor(proposal_df$Response)
