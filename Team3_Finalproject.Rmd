...
```{r}
ggplot(proposal_df, aes(x = cut(Income, breaks = c(5000, 20000, 30000, 40000, 50000), 
                        labels = c("Low Income", "Medium Income", "High Income", "Very High Income"), 
                        include.lowest = TRUE), 
                       fill = factor(Response))) + 
  geom_bar(position = "dodge") + 
  labs(title = "Proposal Acceptance by Income Category", 
       x = "Income Category", 
       y = "Count", 
       fill = "Proposal Response") + 
  theme_minimal()
```
...
subject <- data.frame(   RomanticGestureScore = factor(10, levels = levels(proposal_df$RomanticGestureScore)), CompatibilityScore = factor(7, levels = levels(proposal_df$CompatibilityScore)), CommunicationScore = factor(3, levels = levels(proposal_df$CommunicationScore)), DistanceKM = 0, Height = 173, Income = 0, Age = 23)
